...
function build() {
  ...
  let missingSourceCount = systemPrompt ? 0 : 1;
  ...
  if (missingSourceCount > 0 && fs.existsSync(outPath)) {
    console.warn(
      `[build-kb] ${missingSourceCount} source file(s) missing; keeping existing knowledge-cache.ts instead of overwriting with empty content.`
    );
    return;
  }
